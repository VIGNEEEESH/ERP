...
export function MyOffice() {
    ...
    useEffect(() => {
        const fetchAttendanceAndEmployees = async () => {
            try {
                // Fetch attendance data
                const attendanceResponse = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/erp/attendance/get/attendance/bydate/${formattedDate}`, {
                ...
                // Fetch employee data
                const employeeResponse = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/erp/user/get/all/users`, {
                    headers: { Authorization: "Bearer " + auth.token }
                });
                if (!employeeResponse.ok) {
                    throw new Error(`Failed to fetch employee data: ${employeeResponse.status}`);
                ...
                const statusResponse = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/erp/attendance/status`, {
                    headers: { Authorization: "Bearer " + auth.token }
                });
    ...
    const handleAttendance = async (status) => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/erp/attendance/create/attendance`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    Authorization: "Bearer " + auth.token,
